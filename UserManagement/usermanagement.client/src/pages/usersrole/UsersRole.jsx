...
const UsersRole =() => {
  ...
  const onSearch = async() => {
    await getData('/api/User/GetUserDetailsByUserName/' + userDetails.userName).then((result) => {
          ...
          <div className='container py-3'>
            <h3>Users Role</h3>
                <div className="input-group">
                    <input className="col-md-3" type="text" name="userName" placeholder="Enter user name" value={userDetails.userName} onChange={handleChange}></input>
                    <span class="input-group-addon">&nbsp;</span>
                    <button className="btn btn-primary" onClick={() => onSearch(userDetails.userName)}>
                        Search
                    </button>
                </div>
                <label>Full Name: {userDetails.fullName}</label>
                <label className="col-md-4">User Name: {userDetails.userName}</label>
                <hr></hr>
              ...
              <div className='row'>
                ...
                <div className='col-12'>
                  ...
                  <button className='btn btn-success mt-5' type='submit'>
                    <i className='fa fa-check-circle' aria-hidden='true'></i> Update
                  </button>
                </div>
              </div>
            </form>
